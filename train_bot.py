...
#input_turn = Sequential([
#	Dense(1, input_dim=1),
#	#Input(shape=(1,), dtype='float32', name='turn')
#])

model = Sequential([
	Merge([early_model, input_turn], mode='concat', concat_axis=1),
	Dense(128),	
	LeakyReLU(),
...
def stack_to_input(stack, position):
	return np.take(np.take(stack,
		...
		np.arange(-VISIBLE_DISTANCE,VISIBLE_DISTANCE + 1)+position[1],axis=2,mode='wrap').flatten().astype(np.float32)
...
print("END");
print("TRAINING INPUT:  ", training_input.shape);
print("TRAINING TURN:   ", training_input_turn.shape);
print("TRAINING TARGET: ", training_target.shape);

model.fit([training_input, training_input_turn],training_target,validation_split=0.2,
...
print('STILL accuracy:',model.evaluate([training_input[still_mask],training_input_turn[still_mask]],training_target[still_mask],verbose=0)[1])
print('MOVE accuracy:',model.evaluate([training_input[~still_mask], training_input_turn[~still_mask]],training_target[~still_mask],verbose=0)[1])
